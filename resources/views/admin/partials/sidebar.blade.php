<aside class="left-sidebar sidebar-dark" id="left-sidebar">
          <div id="sidebar" class="sidebar sidebar-with-footer">
            <!-- Aplication Brand -->
            <div class="app-brand">
              <a href="/index.html">
                <img src="{{ asset('theme') }}/images/uniba.png" alt="Mono">
                <span class="brand-name">UNIBA MADURA</span>
              </a>
            </div>
            <!-- begin sidebar scrollbar -->
            <div class="sidebar-left" data-simplebar style="height: 100%;">
              <!-- sidebar menu -->
              <ul class="nav sidebar-inner" id="sidebar-menu">
                
              <li class="active">
                    <a class="sidenav-item-link" href="{{ route('dashboard') }}">
                      <i class="mdi mdi-briefcase-account-outline"></i>
                      <span class="nav-text">Dashboard</span>
                    </a>
                  </li>
                
                  <li class="section-title">
                    Auth
                  </li>

                   <li>
                   <a class="sidenav-item-link" href="{{ route('manajemen_pengguna.index') }}">
                      <i class="mdi mdi-account-group"></i>
                      <span class="nav-text">Manajemen Pengguna</span>
                    </a>
                  </li>
                
                  <li>
                    <a class="sidenav-item-link" href="{{ route('inventaris.index') }}">
                      <i class="mdi mdi-wechat"></i>
                      <span class="nav-text">Data Inventaris</span>
                    </a>
                  </li>
                
                  <li>
                    <a class="sidenav-item-link" href="{{ route('peminjaman.index') }}">
                      <i class="mdi mdi-phone"></i>
                      <span class="nav-text">Data Peminjaman</span>
                    </a>
                  </li>
                

                 
                  <li>
                    <a class="sidenav-item-link" href="{{ route('pengembalian.index') }}">
                      <i class="mdi mdi-calendar-check"></i>
                      <span class="nav-text">Data Pengembalian</span>
                    </a>
                  </li>
                   <li>
                    <a class="sidenav-item-link" href="{{ route('laporan.index') }}">
                      <i class="mdi mdi-calendar-check"></i>
                      <span class="nav-text">Laporan</span>
                    </a>
                  </li>
              </ul>

            </div>

            <div class="sidebar-footer">
              <div class="sidebar-footer-content">
                <ul class="d-flex">
                  <li>
                    <a href="user-account-settings.html" data-toggle="tooltip" title="Profile settings"><i class="mdi mdi-settings"></i></a></li>
                  <li>
                    <a href="#" data-toggle="tooltip" title="No chat messages"><i class="mdi mdi-chat-processing"></i></a>
                  </li>
                </ul>
              </div>
            </div>
          </div>
        </aside>